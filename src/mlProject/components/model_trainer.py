...
class ModelTrainer:
    ...
    def hyperparameter_tuning(self, model, param_grid, X_train, y_train):
        grid_search = GridSearchCV(
            estimator=model, param_grid=param_grid, cv=3, n_jobs=-1, scoring="f1"
        )
        grid_search.fit(X_train, y_train)
        return grid_search.best_estimator_, grid_search.best_params_

    def train_and_log(
        self, model_name, model, params, X_train, X_test, y_train, y_test
    ):
        ...
        with mlflow.start_run(run_name=model_name):
            ...
            logger.info(
                f"Model {model_name} logged and registered as ChurnModel with F1: {f1}"
            )
    ...
    def compare_models(self, results):
        ...
        logger.info(
            f"Best Model Selected: {best_model_info['model_name']} with F1: {best_model_info['metrics']['f1_score']}"
        )
        return best_model_info

    def save_model(self, best_model_info):
        joblib.dump(
            best_model_info["model_obj"],
    ...
    def initiate_model_trainer(self):
        ...
        best_rf, best_rf_params = self.hyperparameter_tuning(
            rf, self.config.rf_params, X_train, y_train
        )
        rf_res = self.train_and_log(
            "RandomForest", best_rf, best_rf_params, X_train, X_test, y_train, y_test
        )
        ...
        xgb_res = self.train_and_log(
            "XGBoost", best_xgb, best_xgb_params, X_train, X_test, y_train, y_test
        )
