...
class PredictionPipeline:
    ...
    def predict(self, data):
        ...
        try:
            ...
            triple_map = {'No': 0, 'No internet service': 0, 'No phone service': 0, 'Yes': 1}
            ...
            df['IS_Fiber optic'] = 1 if data.get('InternetService') == 'Fiber optic' else 0
            df['IS_No'] = 1 if data.get('InternetService') == 'No' else 0

            # PaymentMethod: Bank transfer (automatic), Credit card (automatic), Electronic check, Mailed check
            pm = data.get('PaymentMethod')
            df['PM_Bank transfer (automatic)'] = 1 if pm == 'Bank transfer (automatic)' else 0
            df['PM_Credit card (automatic)'] = 1 if pm == 'Credit card (automatic)' else 0
            ...
            final_columns = [
                ...
                'PM_Electronic check', 'PM_Mailed check', 'TotalServices'
            ]
